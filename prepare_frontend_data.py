...
base_path = '/Users/renat/Desktop/ShipLocate-Reports/database-json'
output_path = '/Users/renat/Desktop/ShipLocate-Reports/reports-frontend/src/lib/real-data.json'
...
def main():
    ...
    print("Filtering loads for FMS Fresh Produce (organizationid = 2, loadstatus = 5)...")
    loads_fms = [l for l in loads_raw_list if l.get('organizationid') == 2 and l.get('loadstatus') == 5]
    ...
    for c in carriers:
        if c.get('organizationid') != 2 or c.get('deleted'):
            continue
        c_name = c['name']
    ...
    active_loads_fms = [l for l in loads_raw_list if l.get('organizationid') == 2 and l.get('loadstatus') in [0, 1, 3, 4]]
    ...
    # 2. Compile RAW dictionary for standalone HTML dashboard
    RAW = {
        "monthly": monthly,
        "weekly": weekly,
        "dow": dow_map,
    ...
    # 3. Read template v6 HTML, replace constants, and save v7 HTML
    template_path = '/Users/renat/Desktop/ShipLocate-Reports/HTML generated by Claude/fms-ops-report-v6.html'
    output_html_path = '/Users/renat/Desktop/ShipLocate-Reports/HTML generated by Claude/fms-ops-report-v7.html'
    
    if os.path.exists(template_path):
        print(f"Reading template HTML {template_path}...")
        with open(template_path, 'r') as f:
            html_content = f.read()
            
        # Replaces
        print("Injecting new dataset JSONs into HTML script...")
        
        # Replace const RAW = {...};
        # Using a pattern that captures the multiline json declaration
        raw_json_str = json.dumps(RAW, indent=2)
        html_content = re.sub(
            r'const\s+RAW\s*=\s*\{.*?\};',
            f'const RAW = {raw_json_str};',
            html_content,
            flags=re.DOTALL
        )
        
        # Replace const CONC = {...};
        conc_json_str = json.dumps(CONC, indent=2)
        html_content = re.sub(
            r'const\s+CONC\s*=\s*\{.*?\};',
            f'const CONC = {conc_json_str};',
            html_content,
            flags=re.DOTALL
        )
        
        # Replace const RETENTION = {...};
        retention_json_str = json.dumps(RETENTION, indent=2)
        html_content = re.sub(
            r'const\s+RETENTION\s*=\s*\{.*?\};',
            f'const RETENTION = {retention_json_str};',
            html_content,
            flags=re.DOTALL
        )
        
        # Replace const PERF = {...};
        perf_json_str = json.dumps(PERF, indent=2)
        html_content = re.sub(
            r'const\s+PERF\s*=\s*\{.*?\};',
            f'const PERF = {perf_json_str};',
            html_content,
            flags=re.DOTALL
        )
        ...
        print(f"Standalone HTML report generated at {output_html_path}")
    else:
